...
export default function AppLayout() {
  ...
  const drawer = <Box height="100%" display="flex" flexDirection="column" bgcolor="#0D4938" color="white">
    <Stack direction="row" alignItems="center" gap={1.4} px={2.5} height={72}>
      ...
      <Box><Typography fontWeight={800} lineHeight={1.15}>Faculty eRepository</Typography><Typography variant="caption" sx={{ opacity: .66 }}>Senior High School</Typography></Box>
    </Stack>
    ...
    <Box p={2}><Box p={1.5} bgcolor="rgba(255,255,255,.08)" borderRadius={3}><Stack direction="row" gap={1.25} alignItems="center"><Avatar src={user?.avatar_url} sx={{ width: 38, height: 38, bgcolor: 'secondary.main' }}>{user?.first_name?.[0]}</Avatar><Box minWidth={0} flex={1}><Typography variant="body2" fontWeight={700} noWrap>{user?.full_name}</Typography><Typography variant="caption" sx={{ opacity: .65 }} noWrap>{user?.position || user?.role?.replaceAll('_', ' ')}</Typography></Box><Tooltip title="Sign out"><IconButton size="small" onClick={logout} sx={{ color: 'white' }}><LogoutRounded fontSize="small" /></IconButton></Tooltip></Stack></Box></Box>
  </Box>
  return <Box display="flex" minHeight="100vh">
    <AppBar position="fixed" color="inherit" elevation={0} sx={{ ml: { md: `${drawerWidth}px` }, width: { md: `calc(100% - ${drawerWidth}px)` }, borderBottom: '1px solid', borderColor: 'divider' }}><Toolbar sx={{ minHeight: '72px !important' }}><IconButton onClick={() => setMobileOpen(true)} sx={{ mr: 1, display: { md: 'none' } }}><MenuRounded /></IconButton><Box flex={1}><Typography variant="h6" fontWeight={750}>{titles[location.pathname] || 'eRepository'}</Typography><Typography variant="caption" color="text.secondary">Secure faculty document workspace</Typography></Box><Chip size="small" label={user?.role?.replaceAll('_', ' ')} sx={{ display: { xs: 'none', sm: 'flex' }, bgcolor: 'primary.light', color: 'primary.dark', fontWeight: 700 }} /></Toolbar></AppBar>
